...
object DbSetup {
  ...
  def setup {
    ...
    val baseLength = createProp(str("Base Length", "Longueur de rallonge de pied de mât"), PropertyType.Int, "cm")
    val varioHead_? = createProp(str("Vario Head?", "Têtière reglable?"), PropertyType.Bool)
    val boomLength = createProp(str("Boom Length", "Longueur au wish"), PropertyType.Int, "cm")
    val numBatten = createProp(str("Battens", "Lattes"), PropertyType.Int)
    val numCams = createProp(str("Cambers", "Cambers"), PropertyType.Int)
    val sailProps = List(surface, mastLength, altMast, luffLength, varioHead_?, boomLength,
                         numBatten, numCams, weight)
